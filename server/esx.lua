...
--- Startup sanity check: without `setr esx:multichar true`, es_extended logs
--- players in by itself on connect and this resource can never take over the
--- selection flow (the selector simply won't open for logged-in players).
CreateThread(function()
    Wait(2500)
    if not W2F.ESX.IsActive() then return end
    if GetConvar('esx:multichar', 'false') ~= 'true' then
        print('^1[w2f-multicharacter] ESX detected but the `esx:multichar` convar is not enabled.^0')
        print('^3[w2f-multicharacter] Add `setr esx:multichar true` to server.cfg (and do not run esx_multicharacter).^0')
    end
end)
