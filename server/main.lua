...
local function isQbxMode()
    return Config.UseQbox and GetResourceState('qbx_core') == 'started'
end

--- qb-core path: active only when neither ESX nor qbx owns the session.
local function isQbCoreMode()
    return not isEsxMode() and not isQbxMode()
        and W2F.QBCore and W2F.QBCore.IsActive and W2F.QBCore.IsActive()
end
...
local function fetchCharactersByLicense(license, license2, source)
    ...
    local rows
    local okQuery, result = pcall(MySQL.query.await,
    ...
    if okQuery then
        ...
        --- qb-core's players table has no `last_logged_out` (and on older
        --- versions no `cid` column either) — retry with the lowest common
        --- subset and derive the slot from charinfo below.
        rows = MySQL.query.await(
            ('SELECT citizenid, charinfo, money, job, metadata, position, gang FROM players WHERE %s'):format(licenseWhere),
            identifiers
        )
    end
    ...
    for i = 1, #rows do
        ...
        local slot = row.cid
        if not slot then
            local info = decodeField(row.charinfo)
            slot = type(info) == 'table' and tonumber(info.cid) or nil
        end
        slot = slot or i
        list[slot] = mapRow(row, slot)
    end
    return list
end
...
lib.callback.register('w2f-multicharacter:server:createCharacter', function(source, payload)
    ...
    local qbxMode = isQbxMode()
    local qbcoreMode = isQbCoreMode()
    if not esxMode and not qbxMode and not qbcoreMode then
        return false, 'A supported framework core (qbx_core, qb-core or es_extended) is required for character creation.'
    end
    ...
    elseif qbcoreMode then
        if W2F.QBCore.GetPlayer(source) then
            return false, 'Already logged into a character.'
        end
    ...
    local runOk, runErr = pcall(withLicenseLock, primaryLicense, 5, function(_locked)
        ...
        if esxMode then
            ...
            if not loginEsxOk then
                ...
                if reason == 'multichar_disabled' then
                    print('^3[w2f-multicharacter] es_extended is not in multichar mode. On ESX 1.13+ this requires a '
                        .. 'resource named esx_multicharacter to exist (a no-code stub works — see esx.md); the '
                        .. '`esx:multichar` convar only applies to older versions.^0')
        ...
        --- QB-family path. qb-core's Player.Login(source, false, newData)
        --- creates the players row from charinfo exactly like qbx_core:Login.
        local loginOk
        if qbcoreMode then
            loginOk = W2F.QBCore.Login(source, nil, result)
        else
            loginOk = exports.qbx_core:Login(source, nil, { charinfo = result })
        end
        if not loginOk then
            createdOk, createdErr = false, 'Failed to create character.'
            return
        ...
        --- Explicit branch (not and/or): a nil GetPlayer on qb-core must not
        --- fall through to the qbx export, which throws when qbx is absent.
        local player
        if qbcoreMode then
            player = W2F.QBCore.GetPlayer(source)
        else
...
lib.callback.register('w2f-multicharacter:server:finishCreation', function(source)
    ...
    elseif isQbxMode() then
        local player = exports.qbx_core:GetPlayer(source)
        citizenid = player and player.PlayerData.citizenid
        exports.qbx_core:Logout(source)
    elseif isQbCoreMode() then
        local player = W2F.QBCore.GetPlayer(source)
        citizenid = player and player.PlayerData.citizenid
        W2F.QBCore.Logout(source)
    end
...
lib.callback.register('w2f-multicharacter:server:saveNewCharacterAppearance', function(source, appearance)
    ...
    local qbxActive = isQbxMode()
    if not qbxActive and not isQbCoreMode() then
        return false, 'qbox_unavailable'
    end

    --- Both QB cores persist new-character skins in `playerskins`
    --- (illenium-appearance and qb-clothing alike), so the write/verify
    --- logic below is shared; only the live-player lookup differs.
    local player
    if qbxActive then
        player = exports.qbx_core:GetPlayer(source)
    else
...
local function deleteCharacterFully(source, citizenid)
    ...
    --- so the core doesn't keep stale data after we wipe the row.
    if isQbxMode() then
        local player = exports.qbx_core:GetPlayer(source)
    ...
    --- Preferred path: the core's own cascade (fires its delete events so
    --- addons clean up too); verified by polling for the row to vanish.
    local cleared = false
    ...
    elseif isQbCoreMode() then
        if W2F.QBCore.DeleteCharacter(source, citizenid) then
            local timeout = GetGameTimer() + 2500
            while GetGameTimer() < timeout do
                local row = MySQL.scalar.await('SELECT 1 FROM players WHERE citizenid = ?', { citizenid })
                if not row then cleared = true break end
...
--- Login for an EXISTING character on ESX and qb-core. Qbox clients use
--- qbx_core's own `qbx_core:server:loadCharacter` callback; the other cores
--- have no equivalent, so the client CharacterLoad service calls this.
--- Logs out any currently active character first (safe relog), then logs
...
lib.callback.register('w2f-multicharacter:server:loadCharacter', function(source, citizenid)
    ...
    if isQbCoreMode() then
        local player = W2F.QBCore.GetPlayer(source)
        if player and player.PlayerData and player.PlayerData.citizenid == citizenid then
            --- Already logged in as this character (retry after a
        ...
        return W2F.QBCore.Login(source, citizenid)
    end
...
--- qb-core equivalent (fired by QBCore.Player.Logout).
