...
local function getPlayerData()
    if W2F.ESX and W2F.ESX.IsActive and W2F.ESX.IsActive() then
        --- Event-tracked identifier, NOT ESX.IsPlayerLoaded() (which only
        --- turns true after the spawn event we fire post-placement —
        --- gating on it here deadlocked every ESX spawn) and NOT
        --- ESX.GetPlayerData().identifier (stale after logout).
        local identifier = W2F.ESX.GetActiveIdentifier and W2F.ESX.GetActiveIdentifier()
    ...
    if W2F.Framework and W2F.Framework.IsQBCore and W2F.Framework.IsQBCore() then
        --- qb-core: GetPlayerData() returns {} pre-login, so the citizenid
        --- check below treats that as "not loaded yet".
        local data = W2F.Framework.GetPlayerData()
        if data and data.citizenid then return data end
        return nil
    end
...
function W2F.CharacterLoad.Load(opts)
    ...
    if not opts.skipLogin then
        ...
        if pd0 and pd0.citizenid == citizenid then
            ...
            --- Only qbx_core provides its own login callback; ESX and qb-core
            --- go through this resource's server:loadCharacter, which routes
            --- to esx:onPlayerJoined / QBCore.Player.Login respectively.
            local useQbxLogin = not esxMode
                and W2F.Qbox and W2F.Qbox.IsActive and W2F.Qbox.IsActive()
            local loginCallback = useQbxLogin
                and 'qbx_core:server:loadCharacter'
                or 'w2f-multicharacter:server:loadCharacter'
