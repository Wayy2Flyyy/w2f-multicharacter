...
local function buildSpawnPreviewCache()
    ...
    for i = 1, #Config.Spawns do
        ...
        if spawn.coords then
            --- Static spawn points are already in shared config — no server
            --- round-trip needed. The old per-spawn callback loop fired 4
            --- awaits back-to-back into resolveSpawnById's 200ms per-source
            --- rate limit, so every call after the first was denied and most
            --- preview targets silently went missing.
            W2F.Spawner.previewTargets[spawn.id] = vector3(spawn.coords.x, spawn.coords.y, spawn.coords.z)
        else
            --- Dynamic entries (type 'last') need the server (saved position).
