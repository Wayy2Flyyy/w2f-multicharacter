...
function W2F.Cleanup.FirePlayerLoadedEvents()
    --- ESX: es_extended fires `esx:playerLoaded` itself during login, BUT in
    --- multichar mode its client handler then blocks on
    --- `DoesEntityExist(ESX.PlayerData.ped)` — the ped is only recorded by a
    --- `playerSpawned` / `esx:onPlayerSpawn` event that the multichar
    --- resource is expected to fire after placing the player. Without this,
    --- ESX.PlayerLoaded never turns true and every esx addon that waits on
    --- it (esx_status, esx_basicneeds, esx_hud, skinchanger loaders) stays
    --- dormant. Mirror the exact sequence es_extended's own non-multichar
    --- spawn path fires.
    if W2F.ESX and W2F.ESX.IsActive and W2F.ESX.IsActive() then
        pcall(TriggerEvent, 'playerSpawned')
        pcall(TriggerEvent, 'esx:onPlayerSpawn')
        TriggerServerEvent('esx:onPlayerSpawn')
        pcall(TriggerEvent, 'esx:restoreLoadout')
        pcall(TriggerEvent, 'esx:loadingScreenOff')
        return
    end
